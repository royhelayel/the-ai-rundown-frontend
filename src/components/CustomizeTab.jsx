...
export default function CustomizeTab({
  ...
  // Form state for creating a new feed
  ...
  return (
    <div style={{ background: light.bg, minHeight: '100dvh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, maxWidth: '600px', margin: '0 auto', width: '100%', padding: '1.5rem 1.25rem', paddingBottom: playerVisible ? '8rem' : '3.5rem' }}>
        ...
        <h1 style={{ margin: '0 0 0.35rem', fontSize: '1.5rem', fontWeight: '900', color: light.text, letterSpacing: '-0.03em' }}>Create Feed</h1>
        <p style={{ margin: '0 0 1.75rem', fontSize: '0.85rem', color: light.textMuted }}>Build named feeds that appear in My Feed.</p>
            ...
            {userFeeds?.length > 0 && (
              <div style={{ marginBottom: '2rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  {userFeeds.map(feed => (
                    <div key={feed.id}
                      style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.85rem 1rem', background: light.bgSub, border: `1px solid ${light.border}`, borderRadius: '12px' }}>
                      ...
                      <button
                        ...
                        style={{ display: 'flex', alignItems: 'center', gap: '0.2rem', padding: '0.3rem 0.6rem', background: `${FEED_COLOR}12`, border: `1px solid ${FEED_COLOR}30`, borderRadius: '999px', color: FEED_COLOR, fontSize: '0.72rem', fontWeight: '700', cursor: 'pointer' }}>
                ...
                {defaultCategories.map(cat => {
                  ...
                  return (
                    <button key={cat} onClick={() => toggleCat(cat)}
                      style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', padding: '0.3rem 0.7rem', borderRadius: '999px', border: `1px solid ${isOn ? color + '60' : light.border}`, background: isOn ? `${color}15` : light.bg, color: isOn ? color : light.textSub, fontSize: '0.8rem', fontWeight: isOn ? '700' : '500', cursor: 'pointer', transition: 'all 0.12s' }}>
