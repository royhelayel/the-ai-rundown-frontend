  ...
  return (
    ...
    <header ref={headerRef} style={{ position: 'sticky', top: 0, zIndex: 50, background: '#f5f5f7' }}>
      <div style={{ maxWidth: 'var(--body-max)', margin: '0 auto' }}>
        ...
        <div style={{ position: 'relative', zIndex: 8, display: 'flex', alignItems: 'center', padding: '11px 16px 13px', gap: 10 }}>
          ...
          <div style={{ position: 'relative' }} ref={pickerRef}>
            <button onClick={() => canPickDay && setPickerOpen(o => !o)} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: 0, background: 'transparent', border: 'none', cursor: canPickDay ? 'pointer' : 'default' }}>
              ...
              <span style={{ fontSize: '0.76rem', fontWeight: 600, color: '#6b7280' }}>{dateLabel}</span>
        ...
        {categories.length > 0 && (
          <div style={{ position: 'relative', zIndex: 3, display: 'flex', alignItems: 'center', borderTop: '1px solid rgba(0,0,0,0.07)' }}>
            ...
            <div style={{ display: 'flex', gap: 8, padding: '16px 16px 14px', minWidth: 'max-content' }}>
              {showAllPill && (
                <button onClick={() => onSelectCategory?.(null)}
                  style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '4px 11px', borderRadius: 8, border: 'none',
                    ...
                    fontSize: '0.76rem', fontWeight: activeCategory === null ? 800 : 600, whiteSpace: 'nowrap', flexShrink: 0, cursor: 'pointer' }}>
                  All
                </button>
              )}
              {categories.map(cat => {
